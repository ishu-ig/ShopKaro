// ─── AdminSubcategory.jsx ────────────────────────────────────────────────────
import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { getSubcategory, deleteSubcategory, updateSubcategory } from "../../Redux/ActionCreators/SubcategoryActionCreators";
...
export default function AdminSubcategory() {
  ...
  const totalCount    = SubcategoryStateData?.length ?? 0;
  const activeCount   = SubcategoryStateData?.filter((i) => i.active).length ?? 0;
  ...
  function updateRecord(_id) {
    const item = SubcategoryStateData.find((s) => s._id === _id);
    if (!item) return;
  ...
  useEffect(() => { dispatch(getSubcategory()); }, [flag]);
          ...
          <div className="page-heading">
            <div className="page-heading-copy">
              ...
              <div>
                ...
                <h1 className="h3 mb-1">Sub Categories</h1>
                <p className="text-muted mb-0">Manage product sub-categories.</p>
              </div>
            </div>
            <div className="heading-actions">
              <Link className="btn btn-primary btn-sm" to="/subcategory/create">
                <i className="bi bi-plus-circle" aria-hidden="true"></i> Add Subcategory
              </Link>
            </div>
          </div>

          <section className="row g-3 mt-2 mb-1" aria-label="Subcategory summary">
            <div className="col-12 col-sm-6 col-xl-4">
              <article className="metric-card text-white">
                <div className="metric-top">
                  <span className="metric-label">Total</span>
                  <span className="metric-icon"><i className="bi bi-grid-3x3-gap-fill"></i></span>
                </div>
                <div className="metric-value">{totalCount}</div>
                <div className="metric-meta"><span>all</span><span>subcategories</span></div>
              </article>
            </div>
            <div className="col-12 col-sm-6 col-xl-4">
              <article className="metric-card text-white">
          ...
          <section className="panel mt-3">
            <div className="panel-header">
              <div>
                ...
                <p className="text-muted mb-0">Search, review, and manage subcategories.</p>
              </div>
                  ...
                  {filteredData.length > 0 ? (
                        ...
                        <td className="text-end">
                          <div className="act-strip">
                            <Link className="act-btn act-btn-edit"
                              to={`/subcategory/update/${item._id}`} data-tip="Edit">
                    ...
                    <tr>
                      <td colSpan="5" className="text-center text-muted py-4">
                        {search ? `No subcategories found for "${search}"` : "No subcategories available."}
                      </td>
                    </tr>
                  )}
                </tbody>
