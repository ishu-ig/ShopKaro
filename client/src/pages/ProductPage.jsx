          ...
          <div className="row g-4 mt-1">
            ...
            <div className="col-lg-7 col-md-12">
              <div className="sk-pp-info">
                ...
                {/* Expected Delivery */}
                <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(200,64,10,0.04)", borderRadius: 12, border: "1px solid rgba(200,64,10,0.1)", display: "flex", alignItems: "center", gap: 10 }}>
                  <i className="fa fa-calendar-check" style={{ color: "#C8400A", fontSize: 16 }} />
                  <div>
                    <div style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7A6E65" }}>Expected Delivery</div>
                    <div style={{ fontSize: "0.88rem", fontWeight: 600, color: "#1C1009", marginTop: 2 }}>
                      Online: by <span style={{ color: "#C8400A" }}>{deliveryDate(4)}</span>
                      &nbsp;·&nbsp;
                      COD: by <span style={{ color: "#C8400A" }}>{deliveryDate(6)}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="sk-tabs">
            ...
            {tab === 'reviews' && (
              <div>
                {/* Review list */}
                {!data.reviews?.length ? (
                  <p style={{ fontSize: 13, color: '#bbb', marginBottom: 24 }}>No reviews yet. Be the first to review this product.</p>
                ) : (
                  <div style={{ marginBottom: 28 }}>
                    {/* Average rating summary */}
                    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, padding: "14px 18px", background: "rgba(200,64,10,0.04)", borderRadius: 14, border: "1px solid rgba(200,64,10,0.08)" }}>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontFamily: "Playfair Display,serif", fontWeight: 900, fontSize: "2.4rem", color: "#C8400A", lineHeight: 1 }}>
                          {(data.reviews.reduce((s, r) => s + r.rating, 0) / data.reviews.length).toFixed(1)}
                        </div>
                        <div style={{ fontSize: "0.7rem", color: "#7A6E65", marginTop: 3 }}>out of 5</div>
                      </div>
                      <div>
                        <div style={{ display: "flex", gap: 3, marginBottom: 4 }}>
                          {[1,2,3,4,5].map(s => (
                            <i key={s} className="fa fa-star" style={{ fontSize: 14, color: (data.reviews.reduce((a, r) => a + r.rating, 0) / data.reviews.length) >= s ? "#f59e0b" : "#e5e7eb" }} />
                          ))}
                        </div>
                        <div style={{ fontSize: "0.78rem", color: "#7A6E65" }}>{data.reviews.length} review{data.reviews.length !== 1 ? "s" : ""}</div>
                      </div>
                    </div>

                    {data.reviews.map((rev, i) => (
                      <div key={i} className="sk-review-item" style={{ padding: "14px 0", borderBottom: "1px solid rgba(200,64,10,0.07)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8, marginBottom: 6 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(200,64,10,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.85rem", color: "#C8400A", flexShrink: 0 }}>
                              {(rev.name || "A")[0].toUpperCase()}
                            </div>
                            <div>
                              <div style={{ fontWeight: 700, fontSize: "0.88rem", color: "#1C1009" }}>{rev.name || "Anonymous"}</div>
                              <div style={{ fontSize: "0.72rem", color: "#7A6E65" }}>{new Date(rev.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</div>
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: 2 }}>
                            {[1,2,3,4,5].map(s => (
                              <i key={s} className="fa fa-star" style={{ fontSize: 12, color: rev.rating >= s ? "#f59e0b" : "#e5e7eb" }} />
                            ))}
                          </div>
                        </div>
                        <p style={{ fontSize: "0.88rem", color: "#555", margin: 0, lineHeight: 1.7 }}>{rev.comment}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {/* Review form */}
                <div style={{ background: "rgba(200,64,10,0.03)", borderRadius: 16, border: "1px solid rgba(200,64,10,0.08)", padding: "20px 22px" }}>
                  <h6 style={{ fontFamily: "Playfair Display,serif", fontWeight: 700, fontSize: "1rem", color: "#1C1009", marginBottom: 16 }}>Write a Review</h6>

                  {/* Star picker */}
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7A6E65", marginBottom: 8 }}>Your Rating</div>
                    <div style={{ display: "flex", gap: 6 }}>
                      {[1,2,3,4,5].map(s => (
                        <i
                          ...
                          style={{ fontSize: 24, cursor: "pointer", color: reviewRating >= s ? "#f59e0b" : "#e5e7eb", transition: "color 0.15s" }}
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Comment box */}
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#7A6E65", marginBottom: 8 }}>Your Comment</div>
                    <textarea
                      rows={4}
                      value={reviewComment}
                      onChange={e => setReviewComment(e.target.value)}
                      placeholder="Share your thoughts about this product..."
                      style={{ width: "100%", borderRadius: 10, border: "1.5px solid rgba(200,64,10,0.15)", padding: "10px 14px", fontSize: "0.88rem", color: "#1C1009", resize: "vertical", outline: "none", fontFamily: "inherit", background: "white", boxSizing: "border-box" }}
                    />
                  </div>

                  {/* Feedback messages */}
                  {reviewError && (
                    <div style={{ fontSize: "0.82rem", color: "#e11d48", background: "rgba(244,63,94,0.07)", padding: "8px 14px", borderRadius: 8, marginBottom: 12 }}>
                      <i className="fa fa-exclamation-circle me-2" />{reviewError}
                    </div>
                  )}
                  {reviewSuccess && (
                    <div style={{ fontSize: "0.82rem", color: "#059669", background: "rgba(16,185,129,0.07)", padding: "8px 14px", borderRadius: 8, marginBottom: 12 }}>
                  ...
                  <button
                    ...
                    style={{ padding: "10px 28px", background: reviewLoading ? "#ccc" : "linear-gradient(135deg,#C8400A,#E86834)", color: "white", border: "none", borderRadius: 50, fontWeight: 700, fontSize: "0.88rem", cursor: reviewLoading ? "not-allowed" : "pointer", transition: "opacity 0.2s", display: "flex", alignItems: "center", gap: 8 }}
                  >
                    {reviewLoading ? <><i className="fa fa-spinner fa-spin" /> Submitting…</> : <><i className="fa fa-paper-plane" /> Submit Review</>}
                  </button>
                </div>
              </div>
            )}
